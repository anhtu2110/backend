...
function change_passAction()
{
    ...
    if (isset($_POST['btn_change_pass'])) {
        if (empty($_POST['old_pass'])) {
            ...
            if (check_pass($_SESSION['user_login'], $_POST['old_pass'])) {
                ...
                if (empty($_POST['re_new_pass'])) {
                    $error['re_new_pass'] = "Vui lòng nhập mật khẩu mới.";
                } else {
                    if (strlen($_POST['re_new_pass']) < 6 or strlen($_POST['re_new_pass']) > 32) {
                        $error['re_new_pass'] = "Mật khẩu mới có độ dài từ 6-32 ký tự.";
                    } else {
                        if (is_password($_POST['re_new_pass'])) {
                            $re_new_pass = $_POST['re_new_pass'];
                        } else {
                            $error['re_new_pass'] = "Mật khẩu mới không đúng định dạng. Vui lòng nhập lại.";
                        }
                    }
                }
            } else {
