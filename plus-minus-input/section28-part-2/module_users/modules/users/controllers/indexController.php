...
function resetAction()
{
    ...
    load_model('reset');
    load('lib', 'validation');
    global $error, $email, $password, $alert;
    ...
    if (!empty($reset_token)) {
        if (check_reset_token($reset_token)) {
            if (isset($_POST['btn_new_pass'])) {
                ...
                if (empty($error)) {
                    if (update_pass($password, $reset_token)) {
                        // redirect("?mod=users&action=login");
?>
                        <script>
                            alert("Đổi mật khẩu thành công. Bạn có thể đăng nhập.");
                            window.location = "?mod=users&action=login";
                        </script>
<?php
                        // $alert['change_pass'] = "Đổi mật khẩu thành công.";
                    }
                    // else {
                    //     $error['change_pass'] = "Cập nhật mật khẩu không thành công";
                    // }
                }
            }
            load_view('newPass');
        } else {
            echo "Yêu cầu lây lại mật khẩu không thành công.";
        }
    } else {
